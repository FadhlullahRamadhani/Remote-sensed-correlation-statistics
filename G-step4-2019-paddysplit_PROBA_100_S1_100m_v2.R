...
for (j in 1:length(period_composite_list)) {
  ...
  folder_ML_classify_fmask <- paste("F:/R-Script-DriveF/ML-PROBA-100-S1-100/CLASSIFY-COMPOSITE-",composite_str,"-25m/",collapse = "",sep = "")
  ...
  for (int_classify in 1:length(period_list_classify)){
    ...
    folder_to <- paste("F:/R-Script-DriveF/ML-PROBA-100-S1-100/CLASSIFY-COMPOSITE-",composite_str,"-25m-PADDY/",collapse = "",sep = "")
    dir.create(folder_to, showWarnings = FALSE)
    folder_to <- paste("F:/R-Script-DriveF/ML-PROBA-100-S1-100/CLASSIFY-COMPOSITE-",composite_str,"-25m-PADDY/",period,"/",collapse = "",sep = "")
    dir.create(folder_to, showWarnings = FALSE)
    folder_to <- paste("F:/R-Script-DriveF/ML-PROBA-100-S1-100/CLASSIFY-COMPOSITE-",composite_str,"-25m-PADDY/",period,"/",model_ML,"/",collapse = "",sep = "")
    dir.create(folder_to, showWarnings = FALSE)
    
    folder_ori <-  paste("F:/R-Script-DriveF/ML-PROBA-100-S1-100/CLASSIFY-COMPOSITE-",composite_str,"-25m/",period,"/",model_ML,collapse = "",sep = "")
    setwd(file.path(folder_ori))
    file_from <- paste(region,"-",period,"_PROBA_S1_ML_utm_COMPOSITE_25m_",composite_str,"_",model_ML,".tif", sep="")

    if (file.exists(file_from)==FALSE) {
      next
    }
    file_to <- paste(folder_to, "/","PADDY_", region,"-",period,"_PROBA_S1_ML_utm_COMPOSITE_25m_",composite_str,"_",model_ML,".tif", sep="")
  ...
  filename <- paste(mainDir_BAT, "/","runBAND_PROBA_S1_paddy_",composite_str,"_",region,".py",sep="")
