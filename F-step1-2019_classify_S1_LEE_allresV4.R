...
mainDir1 <- paste("F:/R-Script-DriveF/S1-ML-2019/ML_S1_100m_2020_v1/svmRadial/",collapse = "",sep = "")

filename <- paste(mainDir1,"WithMasking_LOOCV_ML_S1_100m_2020_v1_svmRadial_noProc_0001.rds", sep="")
  ...
  for (res in resolution) {
    for (region in regencies) { 
      ...
      for (i in 3:diff){
        ...
        JAVA_classify_df <- data.frame(VH1=VH1_raster_df,VH2=VH2_raster_df,
                                       VH3=VH3_raster_df,
                                       stringsAsFactors=FALSE)
        
        colnames(JAVA_classify_df) <- c("B2","B1","B0")
