...
library(readxl)
ML_S1_LEE_2019_v7_WEST <- read_excel("C:/MasseyOffice1/Research/R-Script2019/S1_LEE_DATA_ML_v7_WEST.xlsx")

ML_S1_LEE_2019_v7_WEST$Phase <- as.factor(ML_S1_LEE_2019_v7_WEST$Phase)
set.seed(7)

NR <- filter(ML_S1_LEE_2019_v7_WEST, Phase == "1")
VEG <- filter(ML_S1_LEE_2019_v7_WEST, Phase == "2")
REPR <- filter(ML_S1_LEE_2019_v7_WEST, Phase == "3")
RIPE <- filter(ML_S1_LEE_2019_v7_WEST, Phase == "4")

numberofrows <- c(nrow(NR),nrow(VEG),nrow(REPR),nrow(RIPE))
minrows <- min(numberofrows)
sortrows <- sort(numberofrows,decreasing = TRUE)
minrows <- sortrows[3]+100
#minrows <- 200
index <- sample(1:nrow(NR), 598)
NR_sample <- NR[index, ]
# index <- sample(1:nrow(VEG), 300)
# VEG_sample <- VEG[index, ]
index <- sample(1:nrow(REPR), 598)
REPR_sample <- REPR[index, ]
index <- sample(1:nrow(RIPE), 598)
RIPE_sample <- RIPE[index, ]

ML_S1_LEE_2019_v7_WEST_sampling_down <- rbind(NR_sample, VEG,REPR_sample,RIPE_sample)
ML_S1_LEE_2019_v7_WEST_sampling_down <-  ML_S1_LEE_2019_v7_WEST_sampling_down[sample(1:nrow(ML_S1_LEE_2019_v7_WEST_sampling_down)), ]
#ML_S1_LEE_2019_v7_WEST_sampling_down <- ML_S1_LEE_2019_v7_WEST
rice_fmask <- ML_S1_LEE_2019_v7_WEST_sampling_down
...
# write.csv(train.data,file="ML_S1_LEE_2019_v7_WEST train data ML.csv")
# write.csv(test.data,file="ML_S1_LEE_2019_v7_WEST test data ML.csv")
filename_varImp <- "C:/MasseyOffice1/Research/R-Script2019/S1_LEE_DATA_ML_v7_WEST_USED.csv"
write.csv(rice_fmask, file = filename_varImp)

# rice <-ML_S1_LEE_2019_v7_WEST
...
allModelsList <- c("Phase ~ VV1 + VV2 + VV3 + diff_VV1 + diff_VV2 + diff_VV3 + VH1 + VH2 + VH3 + diff_VH1 + diff_VH2 + diff_VH3",
                   "Phase ~ VH1 + VH2 + VH3 + diff_VH1 + diff_VH2 + diff_VH3",
                   "Phase ~ VV1 + VV2 + VV3 + diff_VV1 + diff_VV2 + diff_VV3",
                   "Phase ~ VV2 + VV3 + diff_VV2 + diff_VV3 + VH2 + VH3 + diff_VH2 + diff_VH3",
                   "Phase ~ VV3 + diff_VV3 + VH3 + diff_VH3",
                   "Phase ~ VV1 + VV3 + diff_VV1 + diff_VV3 + VH1 + VH3 + diff_VH1 + diff_VH3",
                   "Phase ~ VV1 + VV2 + VV3 + VH1 + VH2 + VH3",
                   "Phase ~ VH1 + VH2 + VH3",
                   "Phase ~ VV1 + VV2 + VV3",
                   "Phase ~ VH1",
                   "Phase ~ VH2",
...
model_filter <- c('rpart','rf','svmRadial','avNNet','bayesglm','BstLm','bstTree','C5.0','C5.0Rules','C5.0Tree','cforest','CSimca','ctree','ctree2','dnn','fda','gam','gamLoess','gamSpline','gbm','gcvEarth','glmnet','J48','JRip','kernelpls','kknn','knn','lda','lda2','LMT','LogitBoost','lvq','Mlda','monmlp','multinom','naive_bayes','nnet','OneR','ownn','pam','parRF','PART','partDSA','pcaNNet','pda','PenalizedLDA','pls','rda','rf','RFlda','rpart','rpart1SE','rpart2','rpartScore','rrlda','sda','sdwd','simpls','snn','spls','stepLDA','stepQDA','svmLinear','svmLinear2','svmLinear3','svmPoly','svmRadial','svmRadialCost','svmRadialSigma','treebag','widekernelpls','wsrf')
...
#model_filter <- c('svmRadial')
mainDir <- paste("C:/MasseyOffice1/Research/R-Script2019/ML_S1_LEE_2019_v7_WEST/",collapse = "",sep = "")
...
for (model_ML in model_filter) {
  ...
  for(i_formula in 1:length(allModelsList)) {
    ...
    for(i_preName in 1:1) {
      ...
      filename <- paste(name_fmask,"_",name_data,"_", "ML_S1_LEE_2019_v7_WEST_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      ...
      filename_save <- paste(name_fmask,"_",name_data,"_", "ML_S1_LEE_2019_v7_WEST_",model_ML,"_",preName_current_string,"_",numbering,".rds", sep="")
      ...
      if (any(model_varimp == model_ML) ) {
        ...
        filename_varImp <- paste("varImp_",name_fmask,"_",name_data,"_", "ML_S1_LEE_2019_v7_WEST_",model_ML,"_",preName_current_string,"_",numbering,"_all.csv", sep="")
      ...
      filename_rfc_overall <- paste("ConMat_overall_",name_fmask,"_",name_data,"_", "ML_S1_LEE_2019_v7_WEST_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      filename_rfc_table <- paste("ConMat_table_",name_fmask,"_",name_data,"_", "ML_S1_LEE_2019_v7_WEST_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      filename_rfc_class <- paste("ConMat_class_",name_fmask,"_",name_data,"_", "ML_S1_LEE_2019_v7_WEST_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      ...
      if (any(model_varimp == model_ML) ) {
        allModelsResults_varImp <- varImp(allModelsResults)
        allModelsResults_varImp_1 <- allModelsResults_varImp$importance
        filename_varImp <- paste("varImp_",name_fmask,"_",name_data,"_", "ML_S1_LEE_2019_v7_WEST_",model_ML,"_",preName_current_string,".csv", sep="")
        write.csv(allModelsResults_varImp_1, file = filename_varImp)
      }
