...
library(readxl)
ML_PROBA_100m_2019_v1 <- read_excel("C:/MasseyOffice1/Research/R-Script2019/ML_PROBA_100m_2019_v1.xlsx")

ML_PROBA_100m_2019_v1$Phase <- as.factor(ML_PROBA_100m_2019_v1$Phase)
set.seed(7)

NR <- filter(ML_PROBA_100m_2019_v1, Phase == "1")
VEG <- filter(ML_PROBA_100m_2019_v1, Phase == "2")
REPR <- filter(ML_PROBA_100m_2019_v1, Phase == "3")
RIPE <- filter(ML_PROBA_100m_2019_v1, Phase == "4")
...
#minrows <- 200
index <- sample(1:nrow(NR), 789)
NR_sample <- NR[index, ]
# index <- sample(1:nrow(VEG), 300)
# VEG_sample <- VEG[index, ]
# index <- sample(1:nrow(REPR), minrows)
# REPR_sample <- REPR[index, ]
index <- sample(1:nrow(RIPE), 689)
RIPE_sample <- RIPE[index, ]

ML_PROBA_100m_2019_v1 <- rbind(NR_sample, VEG,REPR,RIPE_sample)
ML_PROBA_100m_2019_v1 <-  ML_PROBA_100m_2019_v1[sample(1:nrow(ML_PROBA_100m_2019_v1)), ]
#ML_PROBA_100m_2019_v1 <- ML_PROBA_100m_2019_v1
rice_fmask <- ML_PROBA_100m_2019_v1
...
# write.csv(train.data,file="ML_PROBA_100m_2019_v1 train data ML.csv")
# write.csv(test.data,file="ML_PROBA_100m_2019_v1 test data ML.csv")
...
allModelsList <- c("Phase ~ RED + NIR + BLUE+ SWIR + NDVI",
                   "Phase ~ RED + NIR + BLUE+ SWIR",
                   "Phase ~ NDVI"
)
library(stringr)
...
mainDir <- paste("C:/MasseyOffice1/Research/R-Script2019/ML_PROBA_100m_2019_v1/",collapse = "",sep = "")
...
for (model_ML in model_filter) {
  ...
  for(i_formula in 1:length(allModelsList)) {
    ...
    for(i_preName in 1:1) {
      ...
      filename <- paste(name_fmask,"_",name_data,"_", "ML_PROBA_100m_2019_v1_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      ...
      filename_save <- paste(name_fmask,"_",name_data,"_", "ML_PROBA_100m_2019_v1_",model_ML,"_",preName_current_string,"_",numbering,".rds", sep="")
      ...
      if (any(model_varimp == model_ML) ) {
        ...
        filename_varImp <- paste("varImp_",name_fmask,"_",name_data,"_", "ML_PROBA_100m_2019_v1_",model_ML,"_",preName_current_string,"_",numbering,"_all.csv", sep="")
      ...
      filename_rfc_overall <- paste("ConMat_overall_",name_fmask,"_",name_data,"_", "ML_PROBA_100m_2019_v1_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      filename_rfc_table <- paste("ConMat_table_",name_fmask,"_",name_data,"_", "ML_PROBA_100m_2019_v1_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      filename_rfc_class <- paste("ConMat_class_",name_fmask,"_",name_data,"_", "ML_PROBA_100m_2019_v1_",model_ML,"_",preName_current_string,"_",numbering,".csv", sep="")
      ...
      if (any(model_varimp == model_ML) ) {
        allModelsResults_varImp <- varImp(allModelsResults)
        allModelsResults_varImp_1 <- allModelsResults_varImp$importance
        filename_varImp <- paste("varImp_",name_fmask,"_",name_data,"_", "ML_PROBA_100m_2019_v1_",model_ML,"_",preName_current_string,".csv", sep="")
        write.csv(allModelsResults_varImp_1, file = filename_varImp)
      }
