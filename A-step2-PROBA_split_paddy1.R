...
mainDir_BANDS <- paste("F:/R-Script-DriveF/ML-PROBA-100/BANDS",sep="")
mainDir_BAT <- paste("F:/R-Script-DriveF/ML-PROBA-100/",sep="")
mainDir_SHP_PADDY <- paste("F:/R-Script-DriveF/ML-PROBA-100/shp/paddy_utm",sep="")

regencies <- c("WEST", "EAST")
bands <- c("RED","NIR","BLUE","SWIR")
...
for (region in regencies) {
  ...
  for (band in bands) {
    ...
    mainDir_BANDS <- paste("F:/R-Script-DriveF/ML-PROBA-100/BANDS-PADDY",sep="")
    dir.create(mainDir_BANDS, showWarnings = FALSE)
    mainDir_BANDS <- paste("F:/R-Script-DriveF/ML-PROBA-100/BANDS-PADDY/",band,sep="")
    dir.create(mainDir_BANDS, showWarnings = FALSE)
    folder_to <- paste("F:/R-Script-DriveF/ML-PROBA-100/BANDS-PADDY/",band,sep="")
    dir.create(folder_to, showWarnings = FALSE)
    
    folder_ori <-  paste("F:/R-Script-DriveF/ML-PROBA-100/BANDS/",band,sep="")
    ...
    temp <- paste(temp, "PROBA_paddy_",region," = \"",band_shp, "\"\r\n",sep="")
    ...
    for (band in files) {
      ...
      if (file.exists(name_band_to)==FALSE) {
        ...
        temp <- paste(temp, "arcpy.gp.ExtractByMask_sa(","from_",band_tif,", ","PROBA_paddy_", region ,", ","to_", band_tif ,")", "\r\n",sep="")      
  ...
  filename <- paste(mainDir_BAT, "/","runBAND_proba_paddy_",region,".py",sep="")
